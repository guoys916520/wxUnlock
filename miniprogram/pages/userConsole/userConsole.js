// pages/userConsole/userConsole.js
const app = getApp()
Page({

  data: {
    openid: getApp().globalData.openId,
    name: "",
    idCard: "",
    sceneId: "",
    apiUrl: getApp().globalData.apiUrl,
    phoneNumber: getApp().globalData.phoneNumber,
    sceneId: ""
  },

  onLoad: function (options) {
    var scene_id = decodeURIComponent(options.scene)
    console.log("二维码参数：" + scene_id)
    this.setData({
      openid: getApp().globalData.openId,
      name: getApp().globalData.name,
      idCard: getApp().globalData.idCard,
      sceneId: scene_id,
    })
    getApp().globalData.sceneId = scene_id
  },
  forName : function(e){
    // console.log( e.detail.value)
    this.setData({
      name: e.detail.value
    })
  },
  forIdCard : function(e){
    // console.log( e.detail.value)
    this.setData({
      idCard: e.detail.value
    })
  },
  faceCheck:function(e){
    if(this.name == "test_bj_dev" && this.idCard == "test_bj_dev"){
    wx.navigateTo({
      url: '/pages/success/success',
    })
  }
    //空值验证、姓名验证、身份证验证
    var code = this.data.idCard
      // console.log(code)
      //身份证号合法性验证 
      //支持15位和18位身份证号
      //支持地址编码、出生日期、校验位验证
        var city = { 11: "北京", 12: "天津", 13: "河北", 14: "山西", 15: "内蒙古", 21: "辽宁", 22: "吉林", 23: "黑龙江 ", 31: "上海", 32: "江苏", 33: "浙江", 34: "安徽", 35: "福建", 36: "江西", 37: "山东", 41: "河南", 42: "湖北 ", 43: "湖南", 44: "广东", 45: "广西", 46: "海南", 50: "重庆", 51: "四川", 52: "贵州", 53: "云南", 54: "西藏 ", 61: "陕西", 62: "甘肃", 63: "青海", 64: "宁夏", 65: "新疆", 71: "台湾", 81: "香港", 82: "澳门", 91: "国外 " };
        var tip = "";
        var pass = true;
      var reg = /^\d{6}(18|19|20)?\d{2}(0[1-9]|1[012])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/;
      if (!code || !code.match(reg)) {
          tip = "身份证号格式错误";
          pass = false;
        }else if (!city[code.substr(0, 2)]) {
          tip = "地址编码错误";
          pass = false;
        }else {
          //18位身份证需要验证最后一位校验位
          if (code.length == 18) {
            code = code.split('');
            //∑(ai×Wi)(mod 11)
            //加权因子
            var factor = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2];
            //校验位
            var parity = [1, 0, 'X', 9, 8, 7, 6, 5, 4, 3, 2];
            var sum = 0;
            var ai = 0;
            var wi = 0;
            for (var i = 0; i < 17; i++) {
              ai = code[i];
              wi = factor[i];
              sum += ai * wi;
            }
            var last = parity[sum % 11];
            if (parity[sum % 11] != code[17]) {
              tip = "校验位错误";
              pass = false;
            }
          }
        }

        var namereg = /^[\u4E00-\u9FA5\uf900-\ufa2d·s]{2,6}$/;
        var n = this.data.name
        if(n.match(namereg)){
          tip = "请填写正确的姓名"
        }
        if(pass){
          if( app.globalData.phoneNumber == ""){
            wx.navigateTo({
              url: '/pages/index/index',
            })
            return
          }
 //跳转页面到核验窗口
 var param = {}
 param.phoneNum = getApp().globalData.phoneNumber
 param.name = this.data.name
 param.idCard = this.data.idCard
 param.wxType = "1"
 param.params = getApp().globalData.param
 wx.request({
  url: 'https://' + app.globalData.apiUrl + '/hotel/wxApi/appCheckFace', //这里就写上后台解析手机号的接口
  //这里的几个参数是获取授权后的加密数据，作为参数传递给后台就行了
  method:'GET',
  header: {
    'content-type': 'application/json'
  },
  data: {
    dp: JSON.stringify(param) + ";1"
},
  success(res) {
    if(res.data.code == "100"){
      wx.showToast({
        title: '核验成功',
        icon: 'success',
        duration: 1000
       })
       wx.navigateTo({
         url: '/pages/success/success',
       })
    }else{
      wx.showToast({
        title: '核验失败',
        icon: 'error',
        duration: 5000
       })
    }
  }
})
        }else{
          wx.showToast({
            title: tip,
            icon: 'error',
            duration: 1000
           })
        }
  }
})